...
WebAuthDialog::WebAuthDialog(QWebEngineWebAuthUxRequest *request,
                             QWidget *parent)
    : QDialog(parent), uxRequest(request),
      uiWebAuthDialog(new Ui::WebAuthDialog) {
  ...
  QAbstractButton *button =
      uiWebAuthDialog->buttonBox->button(QDialogButtonBox::Retry);
  connect(button, &QAbstractButton::clicked, this,
          qOverload<>(&WebAuthDialog::onRetry));
...
WebAuthDialog::~WebAuthDialog() {
  ...
  // selectAccountWidget and it's children will get deleted when scroll area is
  // destroyed
  if (scrollArea) {
    delete scrollArea;
    scrollArea = nullptr;
...
void WebAuthDialog::setupSelectAccountUI() {
  ...
  uiWebAuthDialog->m_description->setText(
      tr("Which passkey do you want to use for ") +
      uxRequest->relyingPartyId() + tr("? "));
  uiWebAuthDialog->m_pinGroupBox->setVisible(false);
  uiWebAuthDialog->m_mainVerticalLayout->removeWidget(
      uiWebAuthDialog->m_pinGroupBox);
  uiWebAuthDialog->buttonBox->button(QDialogButtonBox::Retry)
      ->setVisible(false);
  ...
  uiWebAuthDialog->buttonBox->button(QDialogButtonBox::Cancel)
      ->setVisible(true);
  uiWebAuthDialog->buttonBox->button(QDialogButtonBox::Retry)
      ->setVisible(false);
}
...
void WebAuthDialog::setupCollectPinUI() {
  clearSelectAccountButtons();
  uiWebAuthDialog->m_mainVerticalLayout->addWidget(
      uiWebAuthDialog->m_pinGroupBox);
  ...
  if (pinRequestInfo.reason ==
    ...
    if (pinRequestInfo.reason ==
      ...
      uiWebAuthDialog->m_description->setText(
          tr("Set new PIN for your security key"));
    } else {
      uiWebAuthDialog->m_headingLabel->setText(tr("Change PIN Required"));
      uiWebAuthDialog->m_description->setText(
          tr("Change PIN for your security key"));
    }
...
void WebAuthDialog::onCancelRequest() { uxRequest->cancel(); }
...
void WebAuthDialog::setupErrorUI() {
  ...
  case QWebEngineWebAuthUxRequest::RequestFailureReason::SoftPinBlock:
    errorDescription = tr("The security key is locked because the wrong PIN "
                          "was entered too many times."
  ...
  case QWebEngineWebAuthUxRequest::RequestFailureReason::HardPinBlock:
    ...
    break;
  case QWebEngineWebAuthUxRequest::RequestFailureReason::
      AuthenticatorRemovedDuringPinEntry:
    errorDescription = tr("Authenticator removed during verification. Please "
...
void WebAuthDialog::onRetry() { uxRequest->retry(); }
